...
SOURCES=(
	...
	# Only the files HereBeDragons' own repo actually contains and that this
	# addon's .toc actually loads (HereBeDragons-2.0.lua, HereBeDragons-Pins-2.0.lua).
...
for entry in "${SOURCES[@]}"; do
	...
	if [ -d "$path" ]; then
		...
		echo "SKIPPED $path  (no longer present locally)" >&2
	fi
done
